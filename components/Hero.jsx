  ...
  const actionButtons = [
    {
      text: "Get in touch",
      href: "#contact",
      className: "flex justify-center items-center w-full min-w-[147px] h-full max-h-[48px] bg-white text-neutral-950 hover:text-neutral-50 hover:bg-neutral-800 ease-in-out duration-500 text-sm leadling-[140%] font-medium rounded-lg py-4 uppercase font-archivo"
    },
    {
      text: "Only Proof. no pitch",
      type: "button",
      className: "flex justify-center items-center w-full min-w-[209px] h-full max-h-[48px] bg-neutral-800 text-neutral-50 hover:bg-neutral-700 ease-in-out duration-500 text-sm leadling-[140%] font-medium rounded-lg py-4 uppercase font-archivo"
    }
  ];

  return (
    <section id="hero" className="relative h-auto min-h-[950px] sm:min-h-[950px] lg:min-h-[741px] bg-neutral-950 text-white bg-[url('/backgrounds/HeroGrid.png')] bg-cover bg-center py-8 sm:py-12 lg:py-0">
          ...
          <div className="w-full max-w-[1440px] mx-auto">
            <div className="flex justify-between w-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-20 py-6 sm:py-8 lg:py-10">
              ...
              <div className="flex justify-end">
                <a href="#contact" className="flex justify-center items-center w-full max-w-[117px] h-full max-h-[32px] sm:max-h-[35px] lg:max-h-[37px] text-xs bg-neutral-950 text-white hover:text-neutral-950 hover:bg-neutral-50 ease-in-out duration-500 py-2 px-3 sm:px-4 rounded-lg uppercase font-archivo">
                  Get in Touch
                </a>
              </div>
            </div>
          </div>
          <div className="flex w-full max-w-[1280px] mx-auto mt-[32px] sm:mt-[40px] lg:mt-[52px] px-4 sm:px-6 lg:px-0">
            <div className="flex flex-col lg:flex-row justify-between w-full gap-8 lg:gap-0">
              <div className="flex flex-col gap-3 justify-start w-full lg:max-w-[632px] h-full lg:max-h-[364px] text-center lg:text-left">
                ...
                <div className="flex flex-col w-full sm:flex-row gap-3 sm:gap-4 justify-center lg:justify-start items-center">
                  {actionButtons.map((button, index) => (
                    button.href ? (
                      <a key={index} href={button.href} className={`${button.className} w-full sm:w-fit`}>
                        {button.text}
                      </a>
                    ) : (
                      <button key={index} type={button.type} className={`${button.className} w-full sm:w-fit`}>
                        {button.text}
                      </button>
                    )
                  ))}
